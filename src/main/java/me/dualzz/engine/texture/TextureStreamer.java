...
public class TextureStreamer {
    static TextureData decodeTextureFile(File file) {
        int width = 0;
        int height = 0;
        ...
        try (FileInputStream inputStream = new FileInputStream(file)) {
            ...
            e.printStackTrace();
            System.err.println("Was not able to load texture from file: " + file.getName());
            System.exit(-1);
        }
    ...
    static int loadToOpenGL(TextureData data, TextureBuilder builder) {
        ...
        if (builder.isMipmap()) {
            ...
            if (builder.isAnisotropic() && GLContext.getCapabilities().GL_EXT_texture_filter_anisotropic) {
                GL11.glTexParameterf(GL11.GL_TEXTURE_2D, GL14.GL_TEXTURE_LOD_BIAS, builder.getMipmapBias());
